...
switch func
    ...
    case 'OpenFolderStack' %JS Edit 2022/10/04
...
if PathName~=0
    ...
    elseif strcmpi(FileName(end-2:end),'.dv')
        ...
        FileName = load_smooth_tif(fullfile(PathName, FileName));
    end
...
% JS Edit 2022/10/04
function OpenFolderStack(hMainGui)
global Stack;
global TimeInfo;
global Config;
global Molecule;
...
set(hMainGui.MidPanel.tNoData,'String','Loading Stack...','Visible','on');
set(hMainGui.MidPanel.pView,'Visible','off');
%[FileName,PathName] = uigetfile({'*.stk;*.nd;*.nd2;*.dv;*.zvi;*.tif;*.tiff','Image Stacks (*.stk,*.nd,*.nd2,*.dv,*.zvi,*.tif,*.tiff)'},'Select the Folder for SubStack',FiestaDir.Stack); %open dialog for *.stk files
...
if ~isempty(fOpenStruct)
    ...
    if strcmp(fOpenStruct.Mode,'rSeparateFiles')
        ...
        elseif strcmpi(FileName(end-2:end),'.dv')
            ...
            filetype{1} = 'TIFF';
            % JS Edit 2022/06/16 to do loading in gui
            FileName = load_smooth_tif(fullfile(PathName, FileName));
        end
        options.Block = block;
...
if ~isempty(Stack)
    ...
    hMainGui.Values.PrevScaleMin = hMainGui.Values.ScaleMin;
    hMainGui.Values.PrevScaleMax = hMainGui.Values.ScaleMax;
