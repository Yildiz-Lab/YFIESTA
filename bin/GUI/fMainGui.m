...
global ServerQueue;
global Config;
hMainGui.Region = struct('Area',{},'Check',{},'X',{},'Y',{},'color',{},'ScanData',{}); %JS Edit 2022/07/14 for making scans regions
...
for n = 1:c
    ...
    if isfield(hMainGui.Values,'PrevThresh')
        if length(hMainGui.Values.PrevThresh) == c
            hMainGui.Values.Thresh(n) = hMainGui.Values.PrevThresh(n);
            hMainGui.Values.ScaleMin(n) = hMainGui.Values.PrevScaleMin(n);
            hMainGui.Values.ScaleMax(n) = hMainGui.Values.PrevScaleMax(n);
        else
...
if ~strcmp(get(hMainGui.fig,'Pointer'),'watch')
    ...
    elseif strcmp(get(hMainGui.fig,'SelectionType'),'extend')&&~isempty(Stack)&&~strcmp(hMainGui.CurrentAxes,'none')&&(strcmp(hMainGui.CursorMode,'Normal')||strcmp(hMainGui.CursorMode,'Pan')||(strcmp(hMainGui.CursorMode,'Region')&&all(hMainGui.Values.CursorDownPos==0))||(strcmp(hMainGui.CursorMode,'RectRegion')&&all(hMainGui.Values.CursorDownPos==0)))%||(strcmp(hMainGui.CursorMode,'SegLineRegion')&&all(hMainGui.Values.CursorDownPos==0)) %JS Edit 2022/06/29
...
%         end
        ...
        if strcmp(hMainGui.CurrentAxes,'View')
            ...
            if all(hMainGui.Values.CursorDownPos>0)&&~isempty(Stack)
                ...
                if nRegion>0
                    if (strcmp(hMainGui.CursorMode,'Region')==1 && length(hMainGui.Region(nRegion).X)>5) || (strcmp(hMainGui.CursorMode,'RectRegion')==1 && all(round(cp/10)~=round(hMainGui.Values.CursorDownPos/10))) %|| (strcmp(hMainGui.CursorMode,'SegLineRegion')==1 && length(hMainGui.Region(nRegion).X)>5) %JS Edit 2022/06/29
                        cp=round(cp);
                        nRegion=length(hMainGui.Region);
