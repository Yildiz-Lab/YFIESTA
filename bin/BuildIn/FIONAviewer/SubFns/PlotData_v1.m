...
% JS Edit 2024/12/04 to remove Nan's when plotting fitted lines.
idx = find( ~isnan( handles.currentPlotPSD_Long ) );

if dispShort == 1 % Display short axis

    plot(handles.currentPlotT(idx), real(handles.currentPlotPSD_Short(idx)), ...
        handles.currentstyleY)
...
if dispLong == 1 % Display long axis
    ...
    plot(handles.currentPlotT(idx), real(handles.currentPlotPSD_Long(idx)), ...
        handles.currentstyleX)
...
if length(handles.stepVector) == length(handles.currentPlotT)
    if dispLong == 1
        handles.stepLineHandle = plot( handles.currentPlotT(idx), ...
            handles.stepVector(idx), '-', 'LineWidth', 2, 'Color', [0.2 0.9 0]);
    end
    if dispShort == 1
        handles.offAxisStepHandle = plot( handles.currentPlotT(idx), ...
