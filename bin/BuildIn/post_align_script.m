% A script to do post-processed last minute 2C corrections

%% Part 1: Use offset_by_raw_means_v2 to load and calculate the appropriate change in xy
[dx, dy] = offset_by_raw_means_v2();
% [dx, dy] = offset_by_raw_means_v2('F:\MINFLUX JS\Kinesin 2C\241216\241216_03\241216-110351_minflux_655_LP7p5_bg40k_555_LP4_bg15k_fiesta_annotated.mat');
offset = dx+40;
grid_spacing = 16;
write_pdf = 0;

%% Part 2: Next, choose the annotated molecule file you would like to apply these changes to and apply the corrections
[fname, dir] = uigetfile({'*.mat'},'Select Molecule file');
% [dir, fname, ~] = fileparts('F:\MINFLUX JS\Kinesin 2C\241216\241216_03\241216-110351_minflux_655_LP7p5_bg40k_555_LP4_bg15k_fiesta_annotated.mat');
load(fullfile(dir,fname));

% Molecule should now be in the workspace and editable. So go through each
% molecule and adapt their xy coordinates. Of course only Ch2 need be
% adapted.
% Taken from fMenuOptions > ApplyMultiChannelCorrections
for i = 1:length(Molecule)
    if Molecule(i).Channel == 2
        % for some reason it needs to be a double array
        Molecule(i).Results(:,3) = Molecule(i).Results(:,3) - dx;%*ones(1,length(Molecule(i).Results(:,3)));
        Molecule(i).Results(:,4) = Molecule(i).Results(:,4) - dy;%*ones(1,length(Molecule(i).Results(:,4)));
        % Molecule(i).Results(:,3) = Molecule(i).Results(:,3) + pm*Fx(double(Molecule(i).Results(:,3:4)));
        % Molecule(i).Results(:,4) = Molecule(i).Results(:,4) + pm*Fy(double(Molecule(i).Results(:,3:4)));
        
    end
    
    Molecule(i).Results(:,6) = sqrt( (Molecule(i).Results(:,3) - Molecule(i).Results(1,3)).^2 + (Molecule(i).Results(:,4) - Molecule(i).Results(1,4)).^2 );
    
end

%% Part 3: Choose the files containing the fit molecules (recall the time points are all that is important for these)
[fnames, dir_trace] = uigetfile({'*.mat'},'MultiSelect','on');
%Multiselect allows user to grab more than one file and parse accordingly
savedir = uigetdir(dir_trace, 'Savedir for generated figs and files');

% if only one file is selected, need to package into a cell array so that
% the for loop will work
if class(fnames) == 'char'
    fnames = {fnames};
end

% copied basically all of this from all_in_one_plot_fig_pdf
for i = 1:length(fnames)
    try
        fname = fnames{i};
    catch
        fprintf("Finished combining xy and yx traces. Aborting... \n")
        return
    end
    
    % Now dissect filename connections

    totdata = load(fullfile(dir_trace,fname));
    ch2data = totdata.data;

    % MOLECULE
    % find the molecule in the data Molecule if loaded
    jj = strfind(fname,'_fiona')-1; %nbh or initial
    j = strfind(fname,'_nbh')-1; %if there is a neighbor
    ii = 1;
    if ~isempty(j)
        ii = j+6;
    end
    
    % fname(ii:jj)
    % fname(1:j)

    for k = length(Molecule):-1:1
        % Have it go backwards so that it can only find the total fname.
        % For example. if you were trying to find molecule 627, but there
        % was also a molecule 66273, it would find the higher molecule
        % first. But then upon crossing to 627, it would update m
        % accordingly.

        % Contrastly, if you are trying to find molecule 66273, it will not
        % update when it reaches 627 since the str is not contained in
        % Molecule 627 name. So you are safe. Genius move!
        if ~isempty(strfind(Molecule(k).Name,fname(ii:jj)))
            % Molecule(k).Name
            % length(Molecule(k).Name)
            % strfind(Molecule(k).Name, fname(ii:jj))
            m = k;
        end
        if ~isempty(j)
        if ~isempty(strfind(Molecule(k).Name,fname(1:j)))
            % Molecule(k).Name
            % length(Molecule(k).Name)
            % strfind(Molecule(k).Name, fname(1:j))
            n = k;
        end
        end
    end

    if ~isempty(j) %switch it
        temp = m;
        m = n;
        n = temp;
    end

    if isfield(ch2data, 'time')
        time2 = ch2data.time;
        time2 = ch2data.time(~isnan(ch2data.time));
    else
        time2 = 1:size(ch2data.xy,1);
    end
    
    figgs = figure();
    hold on

    idx2 = find(~isnan(ch2data.trace(:,1)));
    
    % plot(time2-time2(1), ch2data.trace(idx2,1)+offset,'Color',[0 0.7 0],'LineWidth',1.)
    % plot(time2-time2(1), ch2data.trace(idx2,3)+offset,'k','LineWidth',2)
    % 
    % plot(time2-time2(1), ch2data.trace_yx(idx2,1),'Color',[0 0.4 0],'LineWidth',1.)
    % plot(time2-time2(1), ch2data.trace_yx(idx2,3),'k','LineWidth',2)

    plot(time2-time2(1), ch2data.trace(idx2,1)+offset,'Color','blue','LineWidth',1.)
    plot(time2-time2(1), ch2data.trace(idx2,3)+offset,'m','LineWidth',2)

    plot(time2-time2(1), ch2data.trace_yx(idx2,1),'Color',[0 0 0.7],'LineWidth',1.)
    plot(time2-time2(1), ch2data.trace_yx(idx2,3),'m','LineWidth',2)

    ax = gca;
    ax.XGrid = 'off';
    ax.YGrid = 'on';
    yticks(horzcat(fliplr(-grid_spacing:-grid_spacing:ax.YLim(1)), 0:grid_spacing:ax.YLim(2)))


    if contains(fname,'nbh')

            % Calculate the specific dx', dy' (on and off axis)
            % So remember m is associated with the primary axis
            % and n is associated with the projection axis
            
            % Since these are linear corrections, we can apply just linear
            % transformations
            
            % y'' = y' - Rinv(theta)*[dx,dy]
            % where y' is our already existing on and off-axis data.
            
            % We do need the path data to find out what theta is.
            
            theta = atan( (Molecule(m).PathData(end,2) - Molecule(m).PathData(1,2)) / (Molecule(m).PathData(end,1) - Molecule(m).PathData(1,1)));
            
            % Now do Rinv(theta) * [dx dy]'. These are the this molecule specific
            % changes
            dri = [cos(theta), sin(theta); -sin(theta), cos(theta)] * [dx; dy];
            dxi = -dri(1); dyi = -dri(2);
            % [dx dy];
            % -[dxi dyi];
            %make negative since we will add but want to subtract
        
            % Now, nothing changes for this molecule, but everything changes for
            % the neighbor.
        
            % In reality, though, we know what the new dx, dy are and we can just
            % do exactly what is done in all_in_one_plot_fig_pdf. However, we also
            % want to save the dual color file now corrected as determined. Maybe I
            % can just save it all in one
        
            strstart = strfind(fname,'nbh');
            strend = strfind(fname,'fiona');
            fname1 = strcat(fname(1:strstart-1),fname(strend:end));
            % fprintf("nbh")

        % try

            totdata = load(fullfile(dir_trace,fname1));
            ch1data = totdata.data;
    
            if isfield(ch1data, 'time')
                time1 = ch1data.time;
                time1 = ch1data.time(~isnan(ch1data.time));
            else
                time1 = 1:size(ch1data.xy,1);
            end
    
            idx1 = find(~isnan(ch1data.trace(:,1)));
            % time2 is not a typo, want to make sure you are normalizing to the
            % same time
            plot(time1-time2(1), ch1data.trace(idx1,1)+offset+dxi,'Color',[0 0.7 0],'LineWidth',1.)
            plot(time1-time2(1), ch1data.trace(idx1,3)+offset+dxi,'k','LineWidth',2)
    
            plot(time1-time2(1), ch1data.trace_yx(idx1,1)+dyi,'Color',[0 0.4 0],'LineWidth',1.)
            plot(time1-time2(1), ch1data.trace_yx(idx1,3)+dyi,'k','LineWidth',2)
            

        % catch
        %     fprintf("no associated neighbor file ... continuing with just a single channel \n")
        % 
        %     idx1 = [NaN];
        % 
        % end

    else
        fprintf("no associated neighbor file ... continuing with just a single channel \n")

        idx1 = [NaN];


    end
    
    st = zeros(2,2);

    if sum(~isnan(idx1)) > 0

        t1 = max([time1(1),time2(1)]);
        t2 = min([time1(end),time2(end)]);

        midx1a = find(time1 < t2); midx1b = find(time1 > t1);
        midx2a = find(time2 < t2); midx2b = find(time2 > t1);
        idx1 = idx1(intersect(midx1a,midx1b));
        idx2 = idx2(intersect(midx2a,midx2b));

        y1 = (ch1data.trace_yx(idx1(2:end),1) - ch1data.trace_yx(idx1(1:end-1),1)).^2;
        x1 = (ch1data.trace(idx1(2:end),1) - ch1data.trace(idx1(1:end-1),1) ).^2;

        st(1,1) = mean( sqrt(x1 + y1)) / sqrt(2);
        st(1,2) = mean(time1(2:end)-time1(1:end-1))*1000;

        fprintf(strcat("ch1 dx: ", num2str( round(st(1,1),2) ), " nm \n"))
        fprintf(strcat("ch1 dt: ", num2str( round(st(1,2),3) ), " ms \n"))

        y2 = (ch2data.trace_yx(idx2(2:end),1) - ch2data.trace_yx(idx2(1:end-1),1)).^2;
        x2 = (ch2data.trace(idx2(2:end),1) - ch2data.trace(idx2(1:end-1),1) ).^2;
        
        st(2,1) = mean( sqrt(x2 + y2)) / sqrt(2);
        st(2,2) = mean(time2(2:end)-time2(1:end-1))*1000;
    
        fprintf(strcat("ch2 dx: ", num2str( round(st(2,1),2)), " nm \n"))
        fprintf(strcat("ch2 deltat: ", num2str( round(st(2,2),3) ), " ms \n"))

        xycorrect = -[mean(Molecule(m).Results(:,3)) - mean(Molecule(n).Results(:,3)), mean(Molecule(m).Results(:,4)) - mean(Molecule(n).Results(:,4))];
        fprintf(strcat('Mean x, y: (', num2str(round(xycorrect(1),3)) ,', ', num2str(round(xycorrect(2),3)), ')\n'))
    
    else

        y2 = (ch2data.trace_yx(idx2(2:end),1) - ch2data.trace_yx(idx2(1:end-1),1)).^2;
        x2 = (ch2data.trace(idx2(2:end),1) - ch2data.trace(idx2(1:end-1),1) ).^2;
        
        st(2,1) = mean( sqrt(x2 + y2)) / sqrt(2);
        st(2,2) = mean(time2(2:end)-time2(1:end-1))*1000;
    
        fprintf(strcat("ch2 dx: ", num2str( round(st(2,1),2)), " nm \n"))
        fprintf(strcat("ch2 deltat: ", num2str( round(st(2,2),3) ), " ms \n"))


    end

    spatiotemporal_info(:,:,i) = st;
    
    set(0, 'currentFigure', figgs)
    fname(1:end-4)
    title(fname(1:end-4),'Interpreter','none') % why is this not working?
    xlabel("Time (s)")
    ylabel("Distance (nm)")
    
    savename = strcat(fname(1:end-4),"_plot");
    fullsavename = fullfile(savedir, savename);
    saveas(figgs, fullsavename);

    if write_pdf > 0
        saveas(figgs, strcat(fullsavename,'.png'))
        fprintf("Wrote " + savename + '.png' + '\n')
        saveas(figgs, strcat(fullsavename,'.pdf'))
        fprintf("Wrote " + savename + '.pdf' + '\n')   
    end

    close(figgs)

end

